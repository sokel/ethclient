...
func (tx *Transaction) UnmarshalJSON(input []byte) error {
	// call parent unmarshaller
	err := tx.Transaction.UnmarshalJSON(input)
	if err != nil {
		return err
	}
...
type CustomEthereumClient interface {
	...
	// GetTransactionByHash returns transaction header
	GetTransactionByHash(ctx context.Context, txHash common.Hash) (*Transaction, error)
...
func (cc *CustomClient) GetTransactionByHash(ctx context.Context, txHash common.Hash) (*Transaction, error) {
	result := &Transaction{}
	result.Transaction = &types.Transaction{}

	err := cc.c.CallContext(ctx, &result, "eth_getTransactionByHash", txHash)
	if err != nil {
		return nil, err
	}
	if result == nil {
